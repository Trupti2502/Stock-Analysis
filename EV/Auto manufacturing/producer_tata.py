...
def on_message1(message):
    producer1.send('tata', message)
    producer1.flush()

producer1 = KafkaProducer(value_serializer=lambda m: dumps(m).encode("utf-8"), bootstrap_servers=['localhost:9092'])


# url for collecting data of Tata Motors Shares
url = 'https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=TATAMOTORS.BSE&apikey=O6EUNSHQ3DA2FPNM'
r = requests.get(url)
data=r.json()
