...
def on_message1(message):
    producer1.send('powergrid', message)
    producer1.flush()

producer1 = KafkaProducer(value_serializer=lambda m: dumps(m).encode("utf-8"), bootstrap_servers=['localhost:9092'])


# url for collecting the Power Grid company shares data
url = 'https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=POWERGRID.BSE&apikey=3T6PVO8R5LP78CHT'
r = requests.get(url)
data=r.json()
