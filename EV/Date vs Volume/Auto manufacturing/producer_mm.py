...
def on_message1(message):
    producer1.send('mm', message)
    producer1.flush()

producer1 = KafkaProducer(value_serializer=lambda m: dumps(m).encode("utf-8"), bootstrap_servers=['localhost:9092'])


# url to collect data of Mahindra and Mahindra shares
url = 'https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=M&M.BSE&apikey=3T6PVO8R5LP78CHT'
r = requests.get(url)
data=r.json()
